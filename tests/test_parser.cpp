...
int main(){
	...
	std::cout << "look at the bloc chain" << std::endl;
	for (int ia = 0 ; ia < parsed_calcul.size(); ++ia){
		std::cout << "the operator number " << ia << " is at " << parsed_calcul[ia] << " and it links with " << parsed_calcul[ia]->get_ptr_bas1() << " and " << parsed_calcul[ia]->get_ptr_bas2() << std::endl;
	}
	return 0;
}