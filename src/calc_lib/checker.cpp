...
void Checker::is_end_operator() {
	...
	if (!is_input_empty(input_)) {
		int start = auth_.find(input_[0]); // First character of the input string
		int end = auth_.find(input_[input_.size() - 1]); // Last character of the input string

		if (start > 11 && start != 13) {
			// Check if the first operator is invalid. The Minus can be used for a negative number
			checked_ = false;
			return;
		}
...
void Checker::check(){
	if (is_input_empty(input_)){ // If the input is empty, no calcul can be done
        ...
        for(int a=0; a <= input_.size()-1 ; ++a){ //Loop inside the string
                //std::cout << "Current element is  n°" << a  << " : "<< input_[a] << std::endl;
			if (checked_ == false) { // No need to continue if one variable is incorrect
				return;
				}
